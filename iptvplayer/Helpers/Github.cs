...
namespace GithubClient
{
    ...
    public class Github
    {
        ...
        public static async Task<Directory> getRepo(string owner, string uri, string access_token)
        {
            HttpClient client = new HttpClient();
            Directory root = await readDirectory("root", client, String.Format("https://api.github.com/repos/{0}/{1}", owner, uri), access_token);
            client.Dispose();
            return root;
        }
        ...
        private static async Task<Directory> readDirectory(String name, HttpClient client, string uri, string access_token)
        {
            ...
            foreach (FileInfo file in dirContents)
            {
                ...
                { //get the file contents;
                    FileData data = await DownloadFile( access_token, request, file, client);
        ...
        public static async Task<FileData> DownloadFile(string access_token, HttpRequestMessage request, FileInfo file, HttpClient client)
        {
            ...
            downLoadUrl.Headers.Add("Authorization",
                "Basic " + Convert.ToBase64String(System.Text.Encoding.UTF8.GetBytes($"{access_token}:Xamarin-oauth-basic")));
            request.Headers.Add("User-Agent", "lk-github-client");
